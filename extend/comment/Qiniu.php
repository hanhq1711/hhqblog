...
class Qiniu
{
	...
	{
		$conf =[];
		if (config('webinfo.upload_size')) $conf['max_size'] =config('webinfo.upload_size');
		if (config('webinfo.upload_ext')) $conf['exts'] =config('webinfo.upload_ext');
		...
		if (!empty($this->config['exts'])){
			$this->config['exts'] = is_string($this->config['exts']) ? array_map('strtolower',explode(',',$this->config['exts'])):array_map('strtolower',$this->config['exts']);
		}
		return $this;
	}
	//单例实例化\Qiniu\Storage\UploadManager
	private static $instance_manager;
	public static function manager()
	{
		if (self::$instance_manager == NULL){
			self::$instance_manager = new UploadManager();
		}
	...
	{
		$bucket = empty($bucket) ? config('webinfo.qiniu_bucket') :$bucket;
		$auth = new \Qiniu\Auth(config('webinfo.qiniu_accesskey'),config('webinfo.qiniu_secretkey'));
	...
	public function uploadOne($file_path, $prefix = "", $name = null,$ext="", $token = null, $params = null, $mime = 'application/octet-stream', $checkCrc = false)
	{
		...
		if ($ext && empty($prefix)){
			if(in_array(strtolower($ext),['jpg', 'gif', 'png', 'jpeg','bmp'])){
				$prefix = 'images/';
			}elseif (in_array(strtolower($ext),['mp4', 'avi', 'wmv','rm','rmvb','mkv'])){
				$prefix = 'video/';
			}else{
				$prefix = 'files/';
			}
		...
		if (empty($name)){
			$name = base_convert(time() * 1000, 10, 36) . "_" . base_convert(microtime(), 10, 36) . uniqid() .($ext?'.'.$ext:'');
		}
		if(empty($token)) $token = self::token();
		if (empty($mime)) $mime = 'application/octet-stream';
		$this->error = null;
		list($info,$error) = self::manager()->putFile($token, $name, $file_path, $params, $mime, $checkCrc);
		if ($error !==null){
	...
	public function uploadcatch($url,$prefix="",$name="",$ext="",$bucket="")
	{
		...
		if (empty($bucket)) $bucket =config('webinfo.qiniu_bucket');
		$auth = new \Qiniu\Auth(config('webinfo.qiniu_accesskey'),config('webinfo.qiniu_secretkey'));
	...
	public function qiniuDel($key,$bucket='')
	{
		$bucket = empty($bucket) ?config('webinfo.qiniu_bucket'):$bucket;
		$auth = new \Qiniu\Auth(config('webinfo.qiniu_accesskey'),config('webinfo.qiniu_secretkey'));
		$bucketMgr = new BucketManager($auth);
		$error = $bucketMgr->delete($bucket,$key);
		if ($error !=null){
			return false;
		}else{
			return true;
